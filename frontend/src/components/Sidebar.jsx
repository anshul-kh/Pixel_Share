...
const isActiveStyle = 'flex items-center px-5 gap-3 font-extra-bold border-black border-r-2 transition-all duration 200 ease-in-out capitalize'
        ...
        <div className='flex flex-col gap-5'>
          ...
          {categories.slice(0, categories.length - 1).map((category) => (
            <NavLink to={`/category/${category.name}`}
              ...
              key={category.name}
            >
              <img src={category.image} alt="category" className='w-8 h-8 rounded-full shadow-sm' key={category.name} />
              {category.name}
            </NavLink>
          ))}
        </div>
